...
| LUT 构建 | 逆 tone、颜色模型、CAT16 白点适配 | 标准 `.cube` | 源/目标白点、网格尺寸、输出色域、传递函数 |
...
正式整合流程固定使用 CAT16 将实测光源白点下的目标 XYZ 适配到 D65。CAT16 在整合层执行，随后把已经适配的 D65 XYZ 与 `source_white=D65` 交给 LUT 组件，使下游组件的内部适配成为恒等变换。这样无需修改独立的 `chart-lut-builder`，也不会隐式回退到 Bradford。适配方法、源白点、目标白点和矩阵必须写入报告与 provenance。
